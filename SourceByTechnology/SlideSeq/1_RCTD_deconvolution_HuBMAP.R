...
datadirs <- c("/media/NAS2/Users/Kian_NAS2/SlideSeq_data/Batch1", 
              "/media/NAS2/Users/Kian_NAS2/SlideSeq_data/Batch2_redo")
...
ex.lines <- readRDS("ExcludingLines.rds") # a list containing the tissue boundaries for some pucks
...
pucks1 <- list.dirs(datadirs[1], full.names = FALSE)
pucks1 <- pucks1[nchar(pucks1) > 0]
pucks1 <- pucks1[startsWith(pucks1, "Puck_2009")]

pucks2 <- list.dirs(datadirs[2], full.names = FALSE)
pucks2 <- pucks2[nchar(pucks2) > 0]

puckdirs <- paste(datadirs[1], pucks1, sep = '/')
puckdirs <- append(puckdirs, paste(datadirs[2], pucks2, sep = '/'))
puckdirs <- as.list(puckdirs)
names(puckdirs) <- append(pucks1, pucks2)
...
for(puck in names(puckdirs)){
  ...
  if(puck %in% pucks1)
    counts.all[[puck]] <- getPuckCounts(puck, puckdirs[[puck]])
  else
    counts.all[[puck]] <- getPuckCounts2(puck, puckdirs[[puck]])
...
for (l1cl in l1.classes){
  ...
  spat.rna <- SpatialRNA(counts = ssg@raw_exprs, use_fake_coords = T)
  rctd.cl <- create.RCTD(spatialRNA = spat.rna, reference = sn.ref, max_cores = 8,
                          gene_cutoff = 0.0003, gene_cutoff_reg = 0.00035, fc_cutoff = 0.45, fc_cutoff_reg = 0.6)
...
foreach(puck=names(puckdirs)) %do%{
  ind <- if (puck %in% pucks1) 1 else 2
  ssg <- readPuck(puck, puckdirs[[puck]], weights.all.norm, instrs = ginstuc, version = ind)
