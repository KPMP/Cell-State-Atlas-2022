# snCv3 - Plots and Stats (Fig 2 and Ext Data Fig 1) -----------------------------------------------
library(Seurat)
library(SeuratDisk)
library(dplyr)
library(Matrix)
...
#Condition level 2 (Ext Data Fig. 1a)
...
#Region level 2 (Fig 2b, Ext Data Fig. 1a)
...
###By Sample/patient Plots (Ext Data Fig. 2b)
Idents(KBR) <- "patient"
...
row.order <- c("POD","PEC","PT","DTL","ATL","TAL","DCT","CNT","PC","IC","PapE","EC","VSM/P",
               "FIB","IMM","NEU")
prop3 <- prop.table(table(KBR$subclass.l1, KBR$patient), margin = 2)[row.order,col.order]
...
###By Cluster Plots (Ext Data Fig. 2c)
Idents(object = KBR.sub) <- "clusters"
Idents(object = KBR.sub) <- factor(Idents(object = KBR.sub), levels = 1:100)
...
layout(matrix(c(1,2,3,4), nrow = 4, ncol = 1, byrow = TRUE))
col.order <- 1:100
prop1 <- prop.table(table(KBR.sub$experiment.short, KBR.sub$clusters), margin = 2)[,col.order]
barplot(prop1,main = "Experiment Proportions",  names.arg = colnames(prop1), 
        cex.names = 0.5, las=2, col = as.character(exp.cols[rownames(prop1)]))
...
batch.entropy<-table(KBR.sub$experiment.short, KBR.sub$clusters)
entropy <- function(x) {
...
col.order <- 1:100
prop2 <- prop.table(table(KBR.sub$region.l2, KBR.sub$clusters), margin = 2)[,col.order]
...
batch.entropy<-table(KBR.sub$condition.l2, KBR.sub$clusters)
entropy <- function(x) {
  value = entropy::entropy(x ,method='MM',unit='log2')/log2(length(x))
  return(value)
}
